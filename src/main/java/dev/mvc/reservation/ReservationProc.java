...
public class ReservationProc implements ReservationProcInter {
  ...
  public int search_count(HashMap<String, Object> hashMap) {
    int cnt = this.reservationDAO.search_count(hashMap);
    return cnt;
  }
  ...
  public ArrayList<ReservationVO> list_by_managerno_search_paging(ReservationVO reservationVO) {
    ...
    int begin_of_page = (reservationVO.getNow_page() - 1) * Reservation.RECORD_PER_PAGE;
    int start_num = begin_of_page + 1;
    int end_num = begin_of_page + Reservation.RECORD_PER_PAGE;   
    
    reservationVO.setStart_num(start_num);
    reservationVO.setEnd_num(end_num);
    
    ArrayList<ReservationVO> list = this.reservationDAO.list_by_managerno_search_paging(reservationVO);
  ...
  public String pagingBox(int managerno, int now_page, String word, String list_file){
    ...
    int search_count = this.reservationDAO.search_count(hashMap);  // 검색된 레코드 갯수 ->  전체 페이지 규모 파악
    
    // 객체를 만들지 않고 쓰는 메소드 -> static method ( 정적 메소드 ) 라고 한다.
    int total_page = (int)(Math.ceil((double)search_count / Reservation.RECORD_PER_PAGE)); // 전체 페이지 수: (double)1/10 = 0.1 -> 1 (Math.ceil)로 올림이 된다.
