...
//   * @return
//   */
//  @RequestMapping(value="/manager/login.do", method=RequestMethod.GET)  // url가고 GET방식
...
//   * @return
//   */
//  @RequestMapping(value="/manager/login.do", method=RequestMethod.POST)   // 데이터베이스 변경은 post방식 사용
...
//      
//      // session: website 전체에서 공유되는 변수로 서버의 메모리상에 로그아웃 시점까지 유지됨. form은 다음 jsp까지만 유지된다.
//      session.setAttribute("managerno", managerVO_read.getmanagerno()); // 서버의 메모리에 기록
//      session.setAttribute("manager_id", managerVO_read.getId());     // session에 저장하면 어느 jsp에서든 간에 session으로부터 값을 얻어올 수 있다.
...
//  }
  ...
  public ModelAndView logout(HttpSession session){
    ...
    mav.setViewName("redirect:/index.do");  // redirect는 기존에 어떤 정보가 있으면 다 사라지고 다시 시작
                                            // 이전 페이지에서 보낸 정보들이 다 사라진다.
   ...
   */
   ...
   public ModelAndView login_proc(
     ...
     if (cnt == 1) { // 로그인 성공
       ...
       //String id_save = managerVO.getId_save(); // 폼에 입력된 id 저장 여부
       String id = managerVO.getId();              // 폼에 입력된 id
       //String passwd_save = managerVO.getPasswd_save(); // 폼에 입력된 passwd 저장 여부
       String passwd = managerVO.getPasswd();              // 폼에 입력된 passwd 
       
       // -------------------------------------------------------------------
       // id 관련 쿠기 저장
       // -------------------------------------------------------------------
       if (Tool.checkNull(id_save).equals("Y")) { // id를 저장할 경우, Checkbox를 체크한 경우, 체크하지 않으면 null값이 들어온다.
         ...
         response.addCookie(ck_manager_id); // (client의 chrome 관련 폴더에 저장)id 저장
       } else { // N, id를 저장하지 않는 경우, Checkbox를 체크 해제한 경우
         Cookie ck_manager_id = new Cookie("ck_manager_id", "");// 값을 삭제한 Cookie 객체 생성
         ck_manager_id.setPath("/");
         ck_manager_id.setMaxAge(0);
         response.addCookie(ck_manager_id); // (client의 chrome 관련 폴더에 기존에 저장된 Cookie를 덮어 써서 지움)id 저장
       }
       ...
       mav.setViewName("redirect:/manager/msg.do");   // Post-> get // 포스트를 겟으로 바꾸는 것 자체가 정보를 삭제하겠다는 것
     }
