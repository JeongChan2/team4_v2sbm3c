...
public class SellProc implements SellProcInter {
  ...
  public int search_count(HashMap<String, Object> hashMap) {
    int cnt = this.sellDAO.search_count(hashMap);
    return cnt;
  }
  ...
  public ArrayList<Sell_JoinVO> list_by_managerno_search_paging(Sell_JoinVO sell_JoinVO) {
    ...
    int begin_of_page = (sell_JoinVO.getNow_page() - 1) * Sell.RECORD_PER_PAGE;
    int start_num = begin_of_page + 1;
    int end_num = begin_of_page + Sell.RECORD_PER_PAGE;   
    
    sell_JoinVO.setStart_num(start_num);
    sell_JoinVO.setEnd_num(end_num);
    
    ArrayList<Sell_JoinVO> list = this.sellDAO.list_by_managerno_search_paging(sell_JoinVO);
  ...
  public String pagingBox(int managerno, int now_page, String word, String list_file){
    ...
    int search_count = this.sellDAO.search_count(hashMap);  // 검색된 레코드 갯수 ->  전체 페이지 규모 파악
    
    // 객체를 만들지 않고 쓰는 메소드 -> static method ( 정적 메소드 ) 라고 한다.
    int total_page = (int)(Math.ceil((double)search_count / Sell.RECORD_PER_PAGE)); // 전체 페이지 수: (double)1/10 = 0.1 -> 1 (Math.ceil)로 올림이 된다.
                                                                                        // 자료가 1개라고 페이지가 0이면 안되기 때문
    // 전체 그룹 수 : (double)1/10 -> 0.1 -> 1그룹, (double)12/10 -> 1.2 -> 2그룹
    int total_grp = (int)(Math.ceil((double)total_page / Sell.PAGE_PER_BLOCK)); // 전체 그룹  수
    // 현재 그룹 번호 : (double)1/10 -> 0.1 -> 1그룹, (double)12/10 -> 1.2 -> 2그룹
    int now_grp = (int)(Math.ceil((double)now_page / Sell.PAGE_PER_BLOCK));  // 현재 그룹 번호
    
    // 1 group: 1, 2, 3 ... 9, 10
    // 2 group: 11, 12 ... 19, 20
    // 3 group: 21, 22 ... 29, 30
    int start_page = ((now_grp - 1) * Sell.PAGE_PER_BLOCK) + 1; // 특정 그룹의 시작  페이지  
    int end_page = (now_grp * Sell.PAGE_PER_BLOCK);               // 특정 그룹의 마지막 페이지   
...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
    ...
    int _now_page = (now_grp - 1) * Sell.PAGE_PER_BLOCK;  
    ...
    _now_page = (now_grp * Sell.PAGE_PER_BLOCK)+1; //  최대 페이지수 + 1 
