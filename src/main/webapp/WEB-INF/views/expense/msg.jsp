...
String expense_name = (String)request.getAttribute("expense_name");
%>
<div class='title_line'>지출내역 > 알림</div>
<div class='message'>
  <fieldset class='fieldset_basic'>
    <ul>
        ...
        %>
          <li class="li_none">
            <span class="span_success">지출내역을 등록했습니다.</span><br>
            등록된 식당: <%=expense_name %>
          </li>
        ...
        %>
          <li class="li_none">
            <span class="span_fail">지출내역 등록에 실패했습니다.</span><br>
            등록 실패된 식당: <%=expense_name %>
          </li>
        <%  
        }else if(code.equals("delete_success")){
          %>
          <li class='li_none'>
             <span class='span_success'>지출내역 삭제에 성공했습니다.</span><br>
             삭제된 식당: <%=expense_name %>
           </li>
           <%
           ...
           %>
           <li class='li_none'>
             <span class='span_fail'>지출내역 삭제에 실패했습니다.</span><br>
             삭제 실패된 식당: <%=expense_name %>
           </li>
           <%
          }else if(code.equals("update_fail")){
            %>
            <li class='li_none'>
              <span class='span_fail'>지출내역 업데이트에 실패했습니다.</span><br>
              업데이트 실패된 식당: <%=expense_name %>
            </li>
            <%
           }
            %>
