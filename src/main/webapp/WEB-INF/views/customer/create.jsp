...
<script type="text/javascript">
  window.onload = function() {
    ...
    document.querySelector('#height').addEventListener('keypress', (event) => {
    // document.getElementById('passwd').addEventListener('keypress', (event) => {
      if(event.key === 'Enter') {
    ...
    document.querySelector('#weight').addEventListener('keypress', (event) => {
    // document.getElementById('passwd').addEventListener('keypress', (event) => {
      if(event.key === 'Enter') {
  ...
  // jQuery ajax 요청
  // jQuery 형식에서 순수 자바 스크립트 형식으로 변경
  function checkID() {
    ...
    if (id.value.trim().length == 0) {
      ...
      let url = './checkID.do?id='+document.getElementById('id').value;
  ...
  function send() { // 회원 가입 처리
    ...
    if (gender.value.length == 0) {
    	  gender_msg.innerHTML= '성별 입력은 필수입니다.';
    ...
    let height_msg = document.getElementById('height_msg');

    if (height.value.length == 0) {
    	height_msg.innerHTML= '키 입력은 필수입니다.';
    	height_msg.classList.add('span_warning');    // class 적용
    	height.focus();
    ...
    let weight_msg = document.getElementById('weight_msg');

    if (weight.value.length == 0) {
    	weight_msg.innerHTML= '몸무게 입력은 필수입니다.';
...
<c:import url="/menu/top.do" />
  ...
  <form name='frm' id='frm' method='POST' action='./create.do' class="">
    ...
    <div class="form-group">
      ...
      <button type='button' id="btn_checkID" onclick="checkID()" class="btn btn-sm" style='background-color: #80ff00; color: #000040;'>중복확인</button>
      <span id='id_msg'></span>
    </div>   
    ...
    <div class="form-group">
      <label>성명*:
        <input type='text' class="form-control form-control-sm" name='cname' id='cname' value='' required="required" placeholder="성명">
      </label>
    ...
    <div class="form-group">
      <label>나이*:
        <input type='text' class="form-control form-control-sm" name='age' id='age' value='' required="required" placeholder="나이">
      </label>
    </div>
    
    <div class="form-group">
      <label>성별*:
        <input type='text' class="form-control form-control-sm" name='gender' id='gender' value='' required="required" placeholder="성별(남/여)">
      </label>
    </div>    
    
    <div class="form-group">
      <label>키*:
        <input type='text' class="form-control form-control-sm" name='height' id='height' value='' required="required" placeholder="키">
      </label>
    </div>    
    
    <div class="form-group">
      <label>몸무게*:
        <input type='text' class="form-control form-control-sm" name='weight' id='weight' value='' required="required" placeholder="몸무게">
      </label>
    </div>     
    ...
    <div class="form-group"> 
      ...
      <button type="button" id="btn_DaumPostcode" onclick="DaumPostcode()" class="btn btn-sm" style='background-color: #80ff00; color: #000040;'>우편번호 찾기</button>
    </div>  
...
<!-- ------------------------------ DAUM 우편번호 API 종료 ------------------------------ -->
    ...
    <div class="bottom_menu">
      <button type="button" id='btn_send' onclick="send()" class="input_button_form input_submit" >가입</button>
