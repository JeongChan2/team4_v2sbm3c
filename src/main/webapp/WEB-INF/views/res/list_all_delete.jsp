...
<div class="menu_line"></div> 
  ...
  <div id='panel_delete' style="text-align: center;">
    <form name='frm_delete' id='frm_delete' method='post' action='./delete.do'>
      <input type="hidden" name="resno" value="${param.resno }"> <%-- 삭제할 카테고리 번호 --%>

      <c:choose>
        <c:when test="${count_by_resno >= 1 }"> <%-- 자식 레코드가 있는 상황 --%>
          <div class="msg_warning">
            관련 자료 ${count_by_resno } 건이 발견되었습니다.<br>
            관련 자료와 카테고리를 모두 삭제하시겠습니까?
          </div>
            
          <label>관련 카테고리 이름</label>:  ${resVO.resname } 
          <a href="../rescontents/list_by_resno.do?resno=${resVO.resno }&now_page=1" title="관련 카테고리로 이동"><img src='/res/images/link.png'></a>
          &nbsp;      
        ...
        <c:otherwise> <%-- 자식 레코드가 없는 상황 --%>
          ...
          <label>카테고리 이름</label>: ${resVO.resname }
      
          <button type="submit" id='submit' class='btn btn-warning btn-sm' style='height: 28px; margin-bottom: 5px;'>삭제</button>          
        </c:otherwise>
      </c:choose>      

      <button type="button" onclick="location.href='/res/list_all.do'" class='btn btn-info btn-sm' style='height: 28px; margin-bottom: 5px;'>취소</button>
    </form>
  </div>
